...
parser.add_argument("name", nargs="?", default="JsonNumbers", choices=["JsonNumbers", "ArraySortOn"])
name = parser.parse_args().name
with tempfile.TemporaryDirectory() as directory:
    source = Path(directory) / (name + ".as")
...
body += tag(76, struct.pack("<HH", 1, 0) + name.encode("ascii") + b"\0")
body += tag(1, b"") + tag(0, b"")
# JSON is available from Flash Player 11 / SWF version 13 onward.
(fixture / (name + ".swf")).write_bytes(b"FWS\x14" + struct.pack("<I", 8 + len(body)) + body)
