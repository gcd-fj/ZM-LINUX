...
impl CredentialStore for SecretServiceStore {
    async fn save(&self, id: &str, account: &str, password: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            Self::entry(&id, &account)?
                ...
                .map_err(|error| ZmError::Credential(error.to_string()))
        })
        .await
        .map_err(|error| ZmError::Credential(error.to_string()))?
    }
    async fn load(&self, id: &str, account: &str) -> Result<Option<String>> {
        ...
        tokio::task::spawn_blocking(move || match Self::entry(&id, &account)?.get_password() {
            Ok(value) => Ok(Some(value)),
            Err(keyring::Error::NoEntry) => Ok(None),
            Err(error) => Err(ZmError::Credential(error.to_string())),
        })
        .await
        .map_err(|error| ZmError::Credential(error.to_string()))?
    }
    async fn delete(&self, id: &str, account: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(
            move || match Self::entry(&id, &account)?.delete_credential() {
