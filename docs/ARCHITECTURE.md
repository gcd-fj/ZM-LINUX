...
性能诊断区分宿主更新频率、播放器 tick 频率和渲染提交次数，均不作为实际呈现 FPS；各阶段耗时保留最近 1200 个样本。静止的非游戏页面由输入和后台消息唤醒，诊断窗口自动刷新最多每秒一次，手动刷新、复制和停止游戏时生成即时快照。测量方法与实测边界见 [性能采样与验收](PERFORMANCE.md)。
...
认证成功后，缓存校验与官方版本查询并行进行；主 SWF 的已校验字节直接交付播放器，创建播放器时不再次读盘。哈希、补丁和压缩由阻塞工作线程执行，主文件发布、清单切换和运行时命名空间失效包含在同一持锁发布流程中。系统字体目录由应用扫描一次并共享；Ruffle 通用 GPU 描述资源按设备生命周期复用，会话纹理、任务、音频和游戏状态仍独立释放。
