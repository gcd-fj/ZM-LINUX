...
性能诊断区分宿主更新频率、播放器 tick 频率和渲染提交次数，均不作为实际呈现 FPS；各阶段耗时保留最近 1200 个样本，另保留整场会话峰值及发生时间。超过 20ms 的本地任务轮询记录关联资源，保留最近 8 次和全会话最慢一次；该耗时包含解析与游戏回调，并非独立解码计时。静止的非游戏页面由输入和后台消息唤醒，游戏帧等待补偿 egui 对预测帧间隔的扣减，避免截止时间前重复提前唤醒。诊断窗口自动刷新最多每秒一次，手动刷新、复制和停止游戏时生成即时快照。测量方法与实测边界见 [性能采样与验收](PERFORMANCE.md)。
...
造五噩梦 Boss 日志还暴露了空动画帧的滤镜边界问题：对 `Rectangle::INVALID` 扩展模糊或发光滤镜会破坏无效标记，父对象合并后得到数百万像素的边界，导致位图缓存被跳过。`empty-filter-bounds-v1` 仅在边界有效时应用滤镜扩展；回归覆盖空子对象、父边界、遮罩计算与随后出现内容时的正常滤镜扩展。
