...
- 修正奖励列表排序时遇到字符串触发脚本错误的兼容行为，避免这一错误中断页面初始化；自动回归通过，具体活动页面仍待实测确认。
- 美化底部加载提示：使用紧凑的金色指示，合并连续加载状态；资源数量和累计接收量移入悬停详情，减少闪烁和文字跳动。
- 减少加载动画和游戏帧截止时间带来的无效重绘。
- 增强性能诊断，保留整场长帧峰值及慢任务来源，区分服务器响应等待、资源接收和本地处理耗时；可按需开启脚本、预加载、内存回收等内部慢阶段记录。
...
- 已确认藏宝洞等部分战斗场景仍有明显卡顿和掉帧，具体原因尚在定位，本版本没有解决全部战斗停顿。
